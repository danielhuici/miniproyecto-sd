...
// SetTiempo modifica el tiempo del evento
...
// recogePrimerEvento encolado
func (el EventList) leeUltimoEvento() Event {
	if len(el) > 0 {
		return el[el.longitud()-1]
	}
...
// elimina Evento con tiempo
func (el *EventList) eliminaEventoConTiempo(tiempo TypeClock) {
	for _, event := range *el {
		if event.IiTiempo <= tiempo {
